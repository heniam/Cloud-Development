...
describe('concat', () => {
  ...
  it('should concat', () => {
    const result = concat('hen','ok');
    expect(result).to.equal('henok');
  });

  it('should throw an exception if strings are empty', () => {
    expect(() => { concat('hello','') }).to.throw('Strings can not be empty')
  });
